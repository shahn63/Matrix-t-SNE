...
Matrix_tsne<- function(X1, X2, alpha, initial_config = NULL, k=2, perplexity1=5, perplexity2=5, max_iter = 1000, min_cost=0,epoch=100){
  ...
  for (iter in 1:max_iter){
    ...
    for(jj in 1:n2){
      sk2 = apply(Score[,jj,]^2, 1, sum)
      num1 = num1+sk2+sweep(-2 * Score[,jj,] %*% t(Score[,jj,]),2, -t(sk2))
    }
    num1 = 1/(1+num1) 
    
    for(ii in 1:n1){
      sk2 = apply(Score[ii,,]^2, 1, sum)
      num2 = num2+sk2+sweep(-2 * Score[ii,,] %*% t(Score[ii,,]),2, -t(sk2))
    }
    num2 = 1/(1+num2) 
    ...
    for (jj in 1:n2){
      for(ii in 1:n1){
        grads1[ii,jj,] = apply(sweep(-Score[,jj,], 2, -Score[ii,jj,]) * stiffnesses1[,ii],2,sum)
      }
    }
    for (ii in 1:n1){
      for(jj in 1:n2){
        grads2[ii,jj,] = apply(sweep(-Score[ii,,], 2, -Score[ii,jj,]) * stiffnesses2[jj,],2,sum)
      }
    }
    grads= grads1+grads2
